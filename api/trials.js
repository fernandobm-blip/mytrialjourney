export default async function handler(req, res) {
  ...
  try {
    ...
    // SOLO estudios abiertos a accrual / recruiting
    const recruitingOnly = normalized.filter((study) =>
      equalsIgnoreCase(study.status, "RECRUITING")
    );

    const filteredByState = state
      ? recruitingOnly.filter((study) =>
    ...
    return res.status(200).json({
      ...
      meta: {
        ...
        recruitingOnly: recruitingOnly.length,
